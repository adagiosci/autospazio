...
<div>
	...
	/*aqui asignamos el ID de la galería a una variable que se usará para crear las vistas con el shortcode de nextgen*/
		...
		//if($pos !== False){
     			echo do_shortcode('[nggallery template="autospazio" ngg_force_update=53 id='.$mostrar.' override_thumbnail_settings="1" thumbnail_width="174" thumbnail_height="174" thumbnail_crop="1" show_slideshow_link="0"]');
     		//}else{
	...
	if($pos !== False){
		...
		if (!empty($_GET['tag'])){
			//echo $etiquetita;
			echo do_shortcode('[nggtags template="autospazio" ngg_force_update=53 gallery="'.$etiquetita.'" override_thumbnail_settings="1" thumbnail_width="174" thumbnail_height="174" thumbnail_crop="1" show_slideshow_link="0"]');	
			//echo do_shortcode('[album template="autospazio" ngg_force_update=4]﻿');
			}else {
